package ai.beyond.compute.sharded.aira.sia
...
object ShardedSiaAgent extends ShardedMessages {
  def props = Props(new ShardedSiaAgent)
  def name(agentId: Long): String = agentId.toString

  val shardName: String = "sia-compute-agents"
}

class ShardedSiaAgent extends SiaAgent {
  ...
  override def unhandled(msg: Any): Unit = msg match {
    ...
    case SiaAgent.Stop => context.stop(self)
